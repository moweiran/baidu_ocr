...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 增值税发票识别
        findViewById(R.id.value_add_button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!checkTokenStatus()) {
                    return;
    ...
    private void initAccessTokenWithAkSk() {
        ...
        }, c,  "fXzjZ7frV9zyGOoGzsVumgOF", "5aWFBqhG1GQY1jTR6LjulkW5qOOn02xq");
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_TAXIRECEIPT && resultCode == Activity.RESULT_OK) {
            RecognizeService.recTaxireceipt(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(),
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            infoPopText(result);
        ...
        if (requestCode == REQUEST_CODE_VINCODE && resultCode == Activity.RESULT_OK) {
            RecognizeService.recVincode(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(),
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            infoPopText(result);
        ...
        if (requestCode == REQUEST_CODE_TRAINTICKET && resultCode == Activity.RESULT_OK) {
            RecognizeService.recTrainticket(this, FileUtil.getSaveFile(getApplicationContext()).getAbsolutePath(),
                    new RecognizeService.ServiceListener() {
                        @Override
                        public void onResult(String result) {
                            infoPopText(result);
